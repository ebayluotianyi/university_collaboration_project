...
def static_rnn(cell,
               ...
               sequence_length=None,
               scope=None,
               relevanteventsearchkey=None,
               relevanteventsearchvalue=None,
               relevanteventviewkey=None,
               relevanteventviewvalue=None):
  """Creates a recurrent neural network specified by RNNCell `cell`.

  The simplest form of RNN network generated is:

  ```pythonrnn_cell_impl_tianyi
  ...
  with vs.variable_scope(scope or "rnn") as varscope:
    ...
    #index_first_event = relevanteventkey[0]
    #index_final_event = relevanteventvalue[0]
    index_first_event_search = relevanteventsearchkey[0]
    index_final_event_search = relevanteventsearchvalue[0]
    index_first_event_view = relevanteventviewkey[0]
    index_final_event_view = relevanteventviewvalue[0]
    temp_num = 0
    first_hidden_state_search = []
    last_hidden_state_search = []
    first_hidden_state_view = []
    ...
    for time, input_ in enumerate(inputs):
      ...
      #if temp_num == index_final_event:
      #  call_cell = lambda: cell(input_, first_hidden_state + last_hidden_state)
      if temp_num == index_final_event_search:
        call_cell = lambda: cell(input_, first_hidden_state_search + last_hidden_state_search)
      if temp_num == index_final_event_view:
        call_cell = lambda: cell(input_, first_hidden_state_view + last_hidden_state_view)
      ...
      #if temp_num == index_first_event:
      #  first_hidden_state = state
      #if temp_num == index_final_event - 1:
      #  last_hidden_state = state
      if temp_num == index_first_event_search:
        first_hidden_state_search = state
      if temp_num == index_first_event_view:
        first_hidden_state_view = state
      if temp_num == index_final_event_search - 1:
        last_hidden_state_search = state
      if temp_num == index_final_event_view - 1:
        last_hidden_state_view = state
      outputs.append(output)
